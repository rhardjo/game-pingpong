...
//    alert("Player A Wins!");
    if(confirm('Player A wins! Play again?')){
      window.location.reload();  
    } else {
      window.close();
    }
  }
  else if (pingpong.scoreB == 10) {
//    alert("Player B Wins!");
    if(confirm('Player B wins! Play again?')){
      window.location.reload();  
    } else {
      window.close();
    }
...
//  $("#paddleA").css("left",50);
//  $("#paddleB").css("top",70);
//  $("#paddleB").css("left",320);
//}

function scorePoint() {
  var x = 200,
    originalColor = $("#playground").css("background");

  $("#playground").css("background", "white");
  setTimeout(function(){
    $("#playground").css("background", originalColor);
  }, x);
}
